...
Flow:
  CELL 2  upload the CLEAR image (0 EV / normal exposure — windows clearly
          visible), then answer the SCENE HINTS (window size / blinds / count)
          — these auto-tune everything downstream, no code edits needed.
...
SCENE HINTS (new, CELL 2 — answer BEFORE running CELL 3/4):
  WINDOW_SIZE  Small/Medium/Large -> a small real window is never thrown away
               as a "speck"; a large-window scene raises the noise floor a bit.
  HAS_BLINDS   blinds block most of the outdoor glow -> the "is this really a
               window" dark-bracket gate (CELL 5) accepts a much fainter glow.
  WINDOW_COUNT Single/Multiple -> "Single window" keeps only the LARGEST
               surviving mask, silently dropping any leftover false positive
               (e.g. a louvered closet door) without you inspecting anything.
  These three hints apply to EVERY final mask, in BOTH cleanup modes (CELL 5's
  APPLY_CLEANUP on or off) — the defaults (Medium / no blinds / Multiple) behave
  exactly like the original script if you never touch them.
...
# ---------------------------- SCENE HINTS (optional) --------------------------
# Colab shows these as controls on the RIGHT SIDE of this cell (form fields) —
# answer them once, BEFORE running CELL 3/4, and every mask downstream
# auto-tunes to match. Defaults (Medium / no blinds / Multiple) behave exactly
# like the original script if you skip this.
WINDOW_SIZE = "Medium"        #@param ["Small", "Medium", "Large"]
HAS_BLINDS = False            #@param {type:"boolean"}
WINDOW_COUNT = "Multiple windows"   #@param ["Single window", "Multiple windows"]

# Small window -> a small real pane must not be thrown away as a "speck".
# Large window -> raise the floor a bit so small stray blobs don't sneak in.
SPECK_FRAC = {"Small": 0.00015, "Medium": 0.0005, "Large": 0.0012}[WINDOW_SIZE]
# Blinds block most of the outdoor glow -> the dark-bracket "is this really a
# window" gate (CELL 5) must accept a much fainter glow, or a blinded window
# gets rejected as if it were a door.
SEE_GATE = 0.025 if HAS_BLINDS else 0.06
# "Single window" -> after cleanup, keep only the LARGEST surviving blob (kills
# any leftover false positive without you having to inspect the overlay).
# NOTE: if a single window is split into two blobs by a thick centre mullion,
# leave this on "Multiple windows" — else you'd lose half the window.
KEEP_LARGEST_ONLY = (WINDOW_COUNT == "Single window")
print(f"scene hints -> size={WINDOW_SIZE} (speck floor {SPECK_FRAC*100:.3f}%) | "
      f"blinds={HAS_BLINDS} (see-gate {SEE_GATE*100:.1f}%) | "
      f"{'keep LARGEST window only' if KEEP_LARGEST_ONLY else 'keep ALL valid windows'}")


def _component_filter(mask):
    """Drop stray blobs smaller than SPECK_FRAC; if WINDOW_COUNT == 'Single
    window', keep only the single largest surviving blob. Applied to every
    FINAL glass mask in CELL 5 regardless of APPLY_CLEANUP, so the scene hints
    always take effect — even in pure-transfer mode."""
    ncc, lab, st, _ = cv2.connectedComponentsWithStats(mask.astype(np.uint8))
    keep = [i for i in range(1, ncc) if st[i, cv2.CC_STAT_AREA] >= SPECK_FRAC * H * Wd]
    if KEEP_LARGEST_ONLY and keep:
        keep = [max(keep, key=lambda i: st[i, cv2.CC_STAT_AREA])]
...
# the physics cleanup back on if a scene ever needs it. The SCENE HINTS from
# CELL 2 (speck floor / see-gate / single-window) apply to BOTH modes.
...
_missing = [k for k in ("sam3_windows", "oneformer_seg") if k not in RAW]
if _missing:
    raise RuntimeError(
        f"CELL 5 needs RAW{_missing} which isn't set — you skipped a cell. "
        f"Run CELL 3 (SAM 3) and CELL 4 (OneFormer) first, in order, then re-run CELL 5."
    )

APPLY_CLEANUP = False  #@param {type:"boolean"}
# False (default) = PURE TRANSFER: the CELL 3/4 masks are used EXACTLY as they
#                   are — nothing added, nothing removed (beyond the scene-hint
#                   speck floor / single-window filter, which always applies).
# True            = also run the physics cleanup on them (see-through gate,
#                   rod-hung curtain subtraction, dark frame bars).
...
def refine_glass(raw):
    ...
    slats KEPT), drop specks smaller than the scene-hint SPECK_FRAC."""
    ...
    for i in range(1, ncc):
        if st[i, cv2.CC_STAT_AREA] < SPECK_FRAC * H * Wd:
            m[lab == i] = False
    return m
...
if APPLY_CLEANUP:
    kept = drop = 0
    for mi in RAW["sam3_windows"]:
        if float(SEE[mi].mean()) >= SEE_GATE:     # a real window GLOWS in the dark
            raw |= mi; kept += 1
        else:
            drop += 1
...
GLASS["sam3"] = _component_filter(GLASS["sam3"])   # scene-hint speck floor + single-window
...
GLASS["oneformer"] = _component_filter(GLASS["oneformer"])   # scene-hint filter
...
_need = [n for n in ("fused", "darkimg", "GLASS") if n not in globals()]
if _need:
    raise RuntimeError(
        f"CELL 7 needs {_need} which isn't set — you skipped a cell. "
        f"Run CELL 5 (upload darker image) and CELL 6 (upload brackets) first, "
        f"in order, then re-run CELL 7."
    )
...
# ============================== CELL 9 — OPTIONAL: WHITE FRAMES via GPT Image 2
# Same job as CELL 8, but through OpenAI's GPT Image 2 (images.edit) instead of
# Gemini — same prompt, same hard anti-hallucination composite (only the
# FRAME-BAND pixels are taken from GPT Image 2's output; everything else is
# YOUR result, untouched). Run this INSTEAD of or IN ADDITION to CELL 8 to
# compare which model does the cleaner frame recolor.
# Cost: ~$0.02-0.19 per call depending on quality/size. Needs (once):
#   !pip install -q -U openai
# and an OPENAI_API_KEY from https://platform.openai.com/api-keys — add it as
# a Colab secret (key icon, "Notebook access" ON) or paste it when asked.
import io
import base64
...
try:
    from google.colab import userdata
    _okey = userdata.get("OPENAI_API_KEY")
except Exception:
    _okey = os.environ.get("OPENAI_API_KEY")
if not _okey:
    _okey = input("paste your OpenAI API key: ")
_okey = "".join(str(_okey).split())               # kill stray newlines in the key
oclient = OpenAI(api_key=_okey)

# same prompt + negative block as CELL 8 (PROMPT is already built if CELL 8 ran;
# rebuilt here so this cell also works standalone)
PROMPT = ("Do not hallucinate anything. Just change the color of the window frame to white. "
          "Keep blinds, curtains, and everything else exactly as they are. "
          "The window style must stay exactly the same. "
          "The outside view through the window must stay exactly the same, with no change "
...
ok, jb = cv2.imencode(".png", result)              # PNG in (edit endpoint wants png/webp/jpg)
_infile = io.BytesIO(jb.tobytes()); _infile.name = "hdr_result.png"   # SDK reads the filename

print("sending to gpt-image-2 ...")
# NOTE: input_fidelity is intentionally NOT passed — gpt-image-2 always edits
# at high fidelity and the request errors if this parameter is supplied.
_size = "1536x1024" if (Wd / H) >= 1 else "1024x1536"    # landscape vs portrait
resp = oclient.images.edit(
    model="gpt-image-2",
    image=_infile,
    prompt=PROMPT,
    size=_size,
...
if not b64:
    raise RuntimeError("GPT Image 2 returned no image — check API key / quota / content policy.")
gpt_img = cv2.imdecode(np.frombuffer(base64.b64decode(b64), np.uint8), cv2.IMREAD_COLOR)
gpt_img = cv2.resize(gpt_img, (Wd, H), interpolation=cv2.INTER_LANCZOS4)

# ---- SAME hard anti-hallucination composite as CELL 8 — GPT Image 2 pixels
# ONLY in the frame band; everything else is YOUR result, untouched.
g8 = (glass_view > 0.5).astype(np.uint8)
k = max(9, int(Wd * 0.012) | 1)
ring = cv2.dilate(g8, np.ones((k, k), np.uint8)).astype(bool) & ~g8.astype(bool)
near = cv2.dilate(g8, np.ones((k * 4 | 1, k * 4 | 1), np.uint8)).astype(bool)
...
FRAMEB = cv2.GaussianBlur((ring | bars).astype(np.float32), (0, 0), 2)
fb = FRAMEB[..., None]
final_gpt = (result.astype(np.float32) * (1 - fb) + gpt_img.astype(np.float32) * fb)
final_gpt = final_gpt.clip(0, 255).astype("uint8")

cv2.imwrite("gptimage_raw.jpg", gpt_img, [cv2.IMWRITE_JPEG_QUALITY, 92])
cv2.imwrite("hdr_result_whiteframe_gpt.jpg", final_gpt, [cv2.IMWRITE_JPEG_QUALITY, 95])
print(f"frame band = {(FRAMEB > 0.5).mean()*100:.1f}% of image — only these pixels came from GPT Image 2")
try:
    from IPython.display import Image as _Img, display
    print("BEFORE (your HDR result):");   display(_Img("hdr_result.jpg"))
    print("GPT IMAGE 2 raw output (reference only — NOT used directly):"); display(_Img("gptimage_raw.jpg"))
    print("AFTER — white frame, everything else guaranteed untouched:")
    display(_Img("hdr_result_whiteframe_gpt.jpg"))
except Exception:
    pass
files.download("hdr_result_whiteframe_gpt.jpg")
print("done — white-frame result: GPT Image 2 touched ONLY the frame band")
