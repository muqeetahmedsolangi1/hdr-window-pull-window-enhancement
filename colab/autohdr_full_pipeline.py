...
Cells:
  ...
  CELL 5  FUSED-BASED HDR — ALL work happens on the FUSED image (not on the
          CELL-4 result): the new-HDR brightness chain is applied to the fused
          image with the fused colours as the source; PAINTED walls go clean
          white while TILES/MARBLE keep a natural tone and the vanity keeps its
          wood + fused detail; OneFormer guides per-region exposure (±EV table).
...
# WARMTH: OPTIONAL extra manual control. The warm-cast correction itself lives
# in CELL 5 (CAST_TAME — AutoHDR-calibrated), so the default here is 0. Use this
# only to nudge the final look: NEGATIVE = cooler (low/mid-chroma warm surfaces
# only, never strong real colour, never past neutral), POSITIVE = warmer.
WARMTH = 0.0           #@param {type:"number"}   # 0 = keep (default) · -3 = cooler · +2 = warmer
...
def auto_finish(img_u8, target_median=0.73, wb_strength=0.7, warmth=0.0):
    ...
    _ch0 = np.sqrt(a * a + b * b)
    br = (L > 0.75) & (_ch0 < 10)      # TRUE whites only — a bright COLOURED wall
                                       # keeps its colour (it is not a cast to remove)
    ...
    if col.sum() > 1000:
        ...
        a = a * sc; b = b * sc
    if warmth != 0.0:                                    # 4) WARMTH control
        _ch2 = np.sqrt(a * a + b * b)
        w = np.clip((26 - _ch2) / 12, 0, 1)              # walls/cast only — strong real
        if warmth < 0:                                   # colour (wood, ch>26) never moves
            w = w * np.clip(b / 4, 0, 1)                 # cool only what IS warm — and
            b = np.where(b > 0, np.maximum(b + warmth * w, 0.0), b)   # never past neutral
            a = np.where(a > 0, np.maximum(a + 0.35 * warmth * w, 0.0), a)  # (no blue shift)
        else:
            b = b + warmth * w
            a = a + 0.35 * warmth * w
...
if AUTO_CALIBRATE:
    ...
    calibrated = auto_finish(_base, TARGET_MEDIAN, WB_STRENGTH, WARMTH)
...
# Requires CELLS 1-4 to have run once (so the finishing functions exist); run
# CELL 6 too if you want its auto-calibrate applied to every batch result.
import glob
...
for sd in scene_dirs:
    ...
    if "auto_finish" in globals() and AUTO_CALIBRATE:    # CELL 6's normalisation —
        res = auto_finish(res, TARGET_MEDIAN, WB_STRENGTH, WARMTH)   # skipped if CELL 6 wasn't run
