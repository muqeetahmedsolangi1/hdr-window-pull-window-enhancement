...
def main():
    if len(sys.argv) < 2:
        print("usage: python run.py <exposures_folder> [output.jpg] [max_width] [no-enhance]\n"
              "  no-enhance = plain Mertens fusion + sharpen only (checkbox OFF behavior)")
        return
    folder = sys.argv[1]
    ...
    print(f"loaded {len(imgs)} exposures from {folder}  (enhance={enhance})")
    t0 = time.time()
    res = process_brackets(imgs, enhance=enhance, max_width=max_width)
